...
const USD_TO_RUPEE = 64.89;
const RUPEE_TO_USD = 1 / 64.89;
const HEADER_TEXT_PROP = 'Send money to';
const COUNTRY_TEXT = 'India';
...
class CurrencyConverterContainer extends Component {
    ...
    currencyConvertTo(e) {
        e.preventDefault();
        const newFromAmount = (e.target.value * USD_TO_RUPEE).toFixed(2);
        this.setState({
    ...
    currencyConvertFrom(e) {
        e.preventDefault();
        const newToAmount = (e.target.value * RUPEE_TO_USD).toFixed(2);
        this.setState({
